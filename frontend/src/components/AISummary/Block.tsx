  ...
  return (
    <div 
      className={`group relative min-h-[24px] flex items-start rounded transition-all duration-150 ease-in-out
        ${isSelected ? 'bg-blue-50 ring-1 ring-blue-200 shadow-sm' : 'hover:bg-wash'}`}
      ...
      <div className="relative flex-1 py-0.5 px-1">
        <textarea
          ...
          className={`
            ...
            ${block.color === 'gray' ? 'text-muted-ink' : ''}
        ...
        {showCommands && (
          <div 
            ref={commandsRef}
            className="absolute left-0 top-full mt-1 w-64 bg-surface rounded-lg shadow-lg border border-edge py-2 z-50
          ...
          >
            {filteredCommands.map((cmd, index) => (
              <button
                key={cmd.id}
                className={`
                  w-full text-left px-3 py-2 flex items-center space-x-3 hover:bg-wash
                  ${index === selectedCommandIndex ? 'bg-app' : ''}
                `}
                onClick={() => handleCommandSelect(cmd)}
                onMouseEnter={() => setSelectedCommandIndex(index)}
              >
                <span className="flex-shrink-0 w-6 h-6 flex items-center justify-center bg-wash rounded text-muted-ink">
                  {cmd.icon}
                </span>
                <div className="flex-1">
                  <div className="font-medium">{cmd.label}</div>
                  <div className="text-sm text-muted-ink">{cmd.description}</div>
                </div>
              </button>
            ))}
          </div>
