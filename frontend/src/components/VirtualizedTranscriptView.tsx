...
const TranscriptSegment = memo(function TranscriptSegment({
    ...
    return (
        <div id={`segment-${id}`} className={`mb-4 ${speaker ? `border-l-[3px] ${speakerStyle.border} pl-3` : ''}`}>
            <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                    ...
                    {isStreaming ? (
                        <div className="bg-wash border border-edge rounded-lg px-3 py-2">
                            <p className="text-sm text-ink leading-relaxed">{displayText}</p>
                        </div>
                    ) : (
                        <p className="text-sm text-ink leading-relaxed">{displayText}</p>
                    )}
        ...
        <div ref={scrollRef} className="flex flex-col h-full overflow-y-auto px-4 py-2">
            ...
            {segments.length === 0 ? (
                ...
                <motion.div
                    initial={{ opacity: 0 }}
                    animate={{ opacity: 1 }}
                    className="text-center text-muted-ink mt-8"
                >
                    {isRecording ? (
                        <>
                        ...
                        <div ref={loadMoreTriggerRef} className="flex justify-center items-center py-4 mt-2">
                            {isLoadingMore ? (
                                <div className="flex items-center gap-2 text-muted-ink">
                                    <div className="w-4 h-4 border-2 border-edge border-t-gray-600 rounded-full animate-spin" />
