    ...
    <div className="bg-surface rounded-xl border border-edge p-5">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-full bg-wash flex items-center justify-center">
            {icon}
          </div>
          <div>
            <h3 className="font-medium text-ink">{title}</h3>
            <p className="text-sm text-muted-ink">{modelSize}</p>
          </div>
        </div>
        <div>
          {state.status === 'waiting' && (
            <span className="text-sm text-muted-ink">Waiting...</span>
          )}
        ...
        <div className="mt-2 p-3 bg-red-50 border border-red-200 rounded-md">
          ...
          {(title === 'Transcription Engine' || title === 'Summary Engine') && (
            <button
              ...
              className="mt-3 w-full h-9 px-4 bg-ink hover:bg-ink/90 text-app text-sm font-medium rounded-md transition-colors flex items-center justify-center gap-2"
            >
      ...
      <div className="flex flex-col items-center space-y-6">
        ...
        <div className="w-full max-w-lg space-y-4">
          {renderDownloadCard(
            ...
            <Mic className="w-5 h-5 text-muted-ink" />,
            parakeetState,
            '~670 MB'
          )}
          ...
          {renderDownloadCard(
            'Summary Engine',
            <Sparkles className="w-5 h-5 text-muted-ink" />,
            summaryState,
        ...
        <AnimatePresence>
          {parakeetDownloaded && !summaryModelDownloaded && (
            <motion.div
              ...
              className="w-full max-w-lg bg-wash rounded-lg p-4 text-sm text-ink"
            >
              <div className="flex items-start gap-3">
                <Download className="w-5 h-5 text-muted-ink flex-shrink-0 mt-0.5" />
                <div>
        ...
        <div className="w-full max-w-xs space-y-2">
          <Button
            ...
            className="w-full h-11 bg-ink hover:bg-ink/90 text-app disabled:opacity-50 disabled:cursor-not-allowed"
          >
          ...
          {!parakeetDownloaded && (
            <button
              ...
              className="w-full text-sm text-muted-ink hover:text-ink transition-colors"
            >
