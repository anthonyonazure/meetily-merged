  ...
  return (
    <div
      className={cn(
        ...
        isAuthorized ? 'border-ink bg-wash' : isDenied ? 'border-red-300 bg-red-50' : 'bg-surface border-edge'
      )}
    >
      {/* Left side: Icon + Info */}
      <div className="flex items-center gap-3 flex-1 min-w-0">
        ...
        <div
          className={cn(
            ...
            isAuthorized ? 'bg-active' : isDenied ? 'bg-red-100' : 'bg-app'
          )}
        >
          <div className={cn(isAuthorized ? 'text-ink' : isDenied ? 'text-red-500' : 'text-muted-ink')}>{icon}</div>
        </div>
