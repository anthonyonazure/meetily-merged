  ...
  if (loading) {
    return (
      <div className="animate-pulse">
        <div className="h-4 bg-active rounded w-32 mb-2"></div>
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="relative">
          <button
            ...
            className="text-faint hover:text-muted-ink transition-colors"
          >
            <Info className="h-4 w-4" />
          </button>
          {showTooltip && (
            <div className="absolute z-10 left-6 top-0 w-64 p-3 text-xs bg-ink text-white rounded-lg shadow-lg">
          ...
          return (
            <label
              ...
              className={`flex items-start p-3 border rounded-lg transition-all ${
                currentBackend === backend.id
                  ...
                  : 'border-edge hover:border-faint bg-surface'
            ...
            >
              <input
                ...
                disabled={isDisabled}
                className="mt-1 h-4 w-4 text-blue-600 focus:ring-blue-500 border-edge"
              />
              <div className="ml-3 flex-1">
                <div className="flex items-center justify-between">
                  ...
                  {isCoreAudio && (
                    <span className="text-xs font-medium text-muted-ink bg-wash px-2 py-0.5 rounded">
                      Disabled
                    </span>
                  )}
                </div>
                <p className="mt-1 text-xs text-muted-ink">{backend.description}</p>
              </div>
            </label>
          );
        })}
