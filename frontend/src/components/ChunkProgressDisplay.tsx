...
export function ChunkProgressDisplay({
  ...
  const getChunkStatusColor = (status: ChunkStatus['status']) => {
    switch (status) {
      ...
      case 'pending':
      default:
        return 'text-muted-ink bg-app border-edge';
    }
  };

  return (
    <div className={`bg-surface border border-edge rounded-lg p-4 ${className}`}>
      {/* Progress Header */}
      ...
      <div className="grid grid-cols-4 gap-4 mb-4 text-sm">
        <div className="text-center">
          ...
          <div className="text-muted-ink">Completed</div>
        </div>
        ...
        <div className="text-center">
          ...
          <div className="text-muted-ink">Processing</div>
        </div>

        <div className="text-center">
          <div className="text-lg font-semibold text-muted-ink">
            {progress.total_chunks - progress.completed_chunks - progress.processing_chunks - progress.failed_chunks}
          </div>
          <div className="text-muted-ink">Pending</div>
        </div>
        ...
        <div className="text-center">
          ...
          <div className="text-muted-ink">Failed</div>
        </div>
      </div>
      ...
      <div className="space-y-2">
        <h4 className="text-sm font-medium text-muted-ink mb-2">
          ...
          {progress.chunks
            ...
            .map((chunk) => (
              ...
              >
                ...
                {chunk.text_preview && (
                  <div className="mt-1 text-muted-ink text-xs truncate">
                    "{chunk.text_preview}"
                  </div>
                )}
...
export function ChunkProgressMini({ progress, className = '' }: { progress: ProcessingProgress; className?: string }) {
  ...
  return (
    <div className={`bg-app border border-edge rounded-lg p-3 ${className}`}>
      <div className="flex items-center justify-between mb-2">
        <span className="text-sm font-medium text-muted-ink">
          Processing
        </span>
      ...
      <div className="w-full bg-active rounded-full h-1.5 mb-2">
        <div
