        ...
        <div className="bg-surface rounded-lg shadow-xl max-w-4xl w-full max-h-[90vh] overflow-hidden flex flex-col">
          ...
          <div className="flex justify-between items-center p-6 border-b">
            ...
            <button
              ...
              className="text-muted-ink hover:text-muted-ink"
            >
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-8">
            ...
            <div className="border-t pt-8">
              ...
              <div className="space-y-4">
                <div>
                  <label className="block text-sm font-medium text-muted-ink mb-1">
                    Summarization Model
                  </label>
                  <div className="flex space-x-2">
                    <select
                      className="px-3 py-2 text-sm bg-surface border border-edge rounded-md shadow-sm focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500"
                    ...
                    <select
                      className="flex-1 px-3 py-2 text-sm bg-surface border border-edge rounded-md shadow-sm focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500"
                  ...
                  <div>
                    ...
                    <div className="grid gap-4 max-h-[400px] overflow-y-auto pr-2">
                      {models.map((model) => (
                        <div
                          key={model.id}
                          className={`bg-surface p-4 rounded-lg shadow cursor-pointer transition-colors ${modelConfig.model === model.name ? 'ring-2 ring-blue-500 bg-blue-50' : 'hover:bg-wash'
                            }`}
                          onClick={() => setModelConfig((prev: ModelConfig) => ({ ...prev, model: model.name }))}
                        >
                          <h3 className="font-bold">{model.name}</h3>
                          <p className="text-muted-ink">Size: {model.size}</p>
                          <p className="text-muted-ink">Modified: {model.modified}</p>
                        </div>
                      ))}
                    </div>
                  </div>
    ...
    {modals.modelSelector && (
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
        <div className="bg-surface rounded-lg max-w-4xl w-full mx-4 shadow-xl max-h-[90vh] flex flex-col">
          {/* Fixed Header */}
          <div className="flex justify-between items-center p-6 pb-4 border-b border-edge">
          ...
          {/* Fixed Footer */}
          <div className="p-6 pt-4 border-t border-edge flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              <label className="relative inline-flex items-center cursor-pointer">
                ...
                />
                <div className="w-11 h-6 bg-active peer-focus:outline-none peer-focus:ring-2 peer-focus:ring-blue-300 rounded-full peer peer-checked:after:translate-x-full rtl:peer-checked:after:-translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:start-[2px] after:bg-surface after:border-edge after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-blue-600"></div>
              </label>
              <div>
                <p className="text-sm font-medium text-muted-ink">Show Confidence Indicators</p>
                <p className="text-xs text-muted-ink">Display colored dots showing transcription confidence quality</p>
              </div>
            </div>

            <button
              onClick={() => onClose('modelSelector')}
              className="px-4 py-2 text-sm font-medium text-muted-ink bg-wash rounded-md hover:bg-active focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-faint"
            >
