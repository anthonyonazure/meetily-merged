...
//
// This is the one-click official OpenAI lane. The endpoint is intentionally
// hardcoded to the official API; for arbitrary OpenAI-compatible endpoints use
// the generic RemoteProvider ("remote") instead.
...
impl TranscriptionProvider for OpenAIProvider {
    async fn transcribe(
        ...
        if !response.status().is_success() {
            ...
            let msg = format!("OpenAI transcription failed ({}): {}", status, preview);

            // Credential problems get the dedicated variant so the worker can
            // surface a one-shot actionable toast (same as RemoteProvider).
