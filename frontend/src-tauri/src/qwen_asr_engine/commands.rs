...
/// Whether the real Qwen3-ASR vendor engine was compiled into this build.
///
/// Returns false for default builds (stub mode, no `qwen-asr` cargo feature).
/// The frontend uses this to hide the Qwen option entirely; the stub's
/// placeholder output must never reach users.
pub fn engine_available() -> bool {
    unsafe { qwen3_asr_sys::qwen3_asr_is_available() }
}

#[command]
pub async fn qwen_asr_is_available() -> Result<bool, String> {
    Ok(engine_available())
}
...
pub async fn qwen_asr_init() -> Result<(), String> {
    ...
    if !engine_available() {
        return Err(
            "Qwen ASR engine is not compiled into this build (enable the `qwen-asr` cargo feature)"
                .to_string(),
        );
    }
