...
static LISTENER_STATUS: LazyLock<StdMutex<ListenerStatus>> =
    LazyLock::new(|| StdMutex::new(ListenerStatus::default()));
...
/// Coarse listener health only. The upstream implementation also kept a rolling
/// buffer of every keyboard event (keycode, key name, modifier flags) exposed
/// over IPC; that keystroke capture was removed per security audit. Never
/// reintroduce per-key event logging here.
#[derive(Debug, Clone, Serialize)]
...
fn set_listener_debug_state(running: bool, mode: &str, error: Option<String>) {
    if let Ok(mut status) = LISTENER_STATUS.lock() {
        status.listener_running = running;
        status.listener_mode = mode.to_string();
        status.listener_last_error = error;
        if running {
            status.listener_started_at_ms = Some(now_millis());
        }
    }
}
...
async fn transcribe_audio<R: Runtime>(app: &AppHandle<R>, samples_16k: Vec<f32>) -> Result<String, String> {
    ...
    match result {
        ...
        Err(primary_err) => {
            // Fallback sequence for robustness. Qwen is only tried when the
            // engine is actually compiled in (optional `qwen-asr` feature);
            // stub builds skip straight to the Whisper fallback.
            if crate::qwen_asr_engine::commands::engine_available() {
...
pub async fn dictation_restart_listener<R: Runtime>(app: AppHandle<R>) -> Result<String, String> {
    ...
    let status = LISTENER_STATUS
        .lock()
        .map_err(|e| format!("Failed to lock listener status: {e}"))?
        .clone();

    Ok(status.listener_mode)
}
        ...
        for (location, location_name, opt, mode) in attempts {
            ...
            let tap_result = CGEventTap::new(
                ...
                move |_proxy, event_type, event| {
                    ...
                    // SECURITY: this callback sees every keystroke system-wide.
                    // Do not log, buffer, or expose key events here beyond the
                    // hotkey match below (audit-mandated strip of the upstream
                    // debug keystroke buffer).
                    let cfg = hotkey_config_from_atoms();
