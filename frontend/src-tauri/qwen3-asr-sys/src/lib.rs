...
extern "C" {
    /// Check whether the real vendor engine was compiled in.
    ///
    /// Returns `false` for stub builds (crate built without the `vendor`
    /// feature). Stub transcription output is placeholder text and must
    /// never be surfaced to users.
    pub fn qwen3_asr_is_available() -> bool;
